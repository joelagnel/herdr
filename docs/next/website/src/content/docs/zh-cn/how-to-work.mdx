...
你也可以让 Herdr 保持在本地,在其中一个窗格里运行 `ssh`,再在远程 shell 中启动 Claude Code 或 Codex。Herdr 会从实时终端界面识别远程工具、跟踪其状态,并在智能体标签后加上 `(ssh)`。

这不同于 `herdr --remote`:Herdr 服务器和窗格仍在本地,只有 `ssh` 后面的 shell 和智能体在远程。为避免误报,检测会保持谨慎,目前支持 Claude Code 和 Codex。
