...
You can also keep Herdr local, run `ssh` inside one of its panes, and start Claude Code or Codex in that remote shell. Herdr recognizes the remote harness from its live terminal chrome, tracks its state, and marks its agent label with `(ssh)`.

This is different from `herdr --remote`: the Herdr server and pane stay local, while only the shell and agent behind `ssh` are remote. Detection is intentionally conservative and currently covers Claude Code and Codex.
